...
function.statectc <- function(data, incomevar, stateincometaxvar, federalctcvar) {
  cat("\n===== DEBUG: START function.statectc() =====\n")
  cat("Input data: ", nrow(data), " rows\n")
  cat("Unique stateFIPS: ", paste(unique(data$stateFIPS), collapse = ", "), "\n")
  
  # ========== SAFE CHECK FOR locationName ==========
  has_location <- "locationName" %in% names(data)
  if (has_location) {
    cat("===== DEBUG: DC Subset BEFORE processing (with locationName) =====\n")
    print(head(data[data$stateFIPS == 11, c("locationName", "stateFIPS", "FilingStatus", incomevar, stateincometaxvar, federalctcvar)], 10))
  } else {
    cat("===== DEBUG: DC Subset BEFORE processing (no locationName) =====\n")
    print(head(data[data$stateFIPS == 11, c("stateFIPS", "FilingStatus", incomevar, stateincometaxvar, federalctcvar)], 10))
  }
  ...
  cat("===== DEBUG: BEFORE AGE RULE MERGE =====\n")
  print(head(data[data$stateFIPS == 11, c("stateFIPS", "FilingStatus")], 10))
  cat("Joining to state_rules...\n")
  
  data <- left_join(data, state_rules, by = c("stateFIPS", "FilingStatus"))
  
  cat("===== DEBUG: AFTER AGE RULE MERGE =====\n")
  print(head(data[data$stateFIPS == 11, c("stateFIPS", "FilingStatus", "AgeofDependentMin", "AgeofDependentMax", "MaxDependent")], 10))
  cat("NAs in AgeofDependentMin: ", sum(is.na(data$AgeofDependentMin)), "\n")
  ...
  cat("===== DEBUG: DC numeligiblekids summary =====\n")
  print(summary(data$numeligiblekids[data$stateFIPS == 11]))
  ...
  cat("===== DEBUG: DC in data_standard =====\n")
  print(head(data_standard[data_standard$stateFIPS == 11, c("stateFIPS", "FilingStatus", "numeligiblekids", "income.base")], 10))
  ...
  cat("===== DEBUG: DC after full merge (standard logic) =====\n")
  print(head(data_standard[data_standard$stateFIPS == 11, c("income.base", "numeligiblekids", "ValueBin1", "Refundable")], 10))
  ...
  if (nrow(data_standard) > 0) {
    ...
    cat("===== DEBUG: DC assigned_bins =====\n")
    print(assigned_bins[data_standard$stateFIPS == 11])
    ...
    cat("===== DEBUG: DC Rows After Assignment =====\n")
    print(data_standard[data_standard$stateFIPS == 11, c("income.base", "numeligiblekids", "value.statectc")])
    ...
    if (any(phaseout_mask)) {
      cat("===== DEBUG: Applying phaseout for standard states =====\n")
  ...
  cat("===== DEBUG: Final DC Results =====\n")
  print(head(data[data$stateFIPS == 11, c("income.base", "numeligiblekids", "value.statectc")], 10))
  
  cat("\n===== DEBUG: END function.statectc() =====\n")
