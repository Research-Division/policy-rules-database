...
function.snapBenefit<-function(data){
  ...
  if(length(futureYrs)>0) {snapData<-snapData %>% rbind(expand)}
    ...
    data$fail_grossIncomeTest[data$not_categ_elig_tanf==FALSE|data$not_categ_elig_ssi==FALSE]<- FALSE
...
function.fedinctax<-function(data
  ...
  if ("ruleYear" %in% colnames(data)) {
    data$ruleYear[data$ruleYear > 2026] <- 2026
  }
...
function.stateinctax<-function(data
  ...
  if(2026 %in% unique(data$ruleYear) & "AL" %in% unique(data$stateAbbrev)){
    
    # Condition for standard deduction phase out
    subset0 <- data$income.base > data$AL_PhaseOutThreshold1 & data$stateAbbrev=="AL" & data$ruleYear==2026
    ...
    subset1 <- data$income.base > data$AL_DepExReductionThreshold1 & data$stateAbbrev=="AL" & data$ruleYear==2026
    
    data$DependentExemption[subset1] <- data$AL_ReducedDependentExemption1[subset1]
    
    subset2 <- data$income.base > data$AL_DepExReductionThreshold2 & data$stateAbbrev=="AL" & data$ruleYear==2026
    
    data$DependentExemption[subset2] <- data$AL_ReducedDependentExemption2[subset2]
  }
  
  # f)
  if(2026 %in% unique(data$ruleYear) & "AZ" %in% unique(data$stateAbbrev)){
  ...
  # g & ll)
  if(2026 %in% unique(data$ruleYear) & "AR" %in% unique(data$stateAbbrev)){
    
    subset0 <- data$income.base < data$AR_Threshold1 & data$stateAbbrev=="AR" & data$ruleYear==2026
    ...
    subset1 <- (data$income.base >= data$AR_Threshold1) &
      (data$income.base <  data$AR_Threshold2) &
      data$FilingStatus %in% c(1,3,4) &
      data$stateAbbrev == "AR" &
      data$ruleYear == 2026
    ...
    if (any(subset1)) {
      ...
      adj <- data$AR_BracketAdjustment[subset1]
      ph  <- data$AR_Phaseout1[subset1]
      th1 <- data$AR_Threshold1[subset1]
      pe  <- data$PersonalExemption[subset1]
      ...
      data$PersonalExemption[subset1] <-
        pe + adj - (data$income.base[subset1] - th1) * ph
    }
    
  }
  
  # k)
  if(2026 %in% unique(data$ruleYear) & ("CA" %in% unique(data$stateAbbrev))){
    
    # Condition for standard deduction phase out
    subset0 <- data$income.base > data$CA_ExemptionThreshold1 & data$stateAbbrev=="CA" & data$ruleYear==2026
  ...
  if(2026 %in% unique(data$ruleYear) & "CT" %in% unique(data$stateAbbrev)){
    
    # Condition for standard deduction phase out
    subset0 <- data$income.base > data$CT_ExemptThreshold1 & data$stateAbbrev=="CT" & data$ruleYear==2026
  ...
  # v)
  if(2026 %in% unique(data$ruleYear) & "IL" %in% unique(data$stateAbbrev)){
  ...
  # z & aa)
  if(2026 %in% unique(data$ruleYear) & "MD" %in% unique(data$stateAbbrev)){
    ...
    # Condition for standard deduction phase out
    subset0 <- data$income.base > data$MD_ExemptThreshold1 & data$stateAbbrev=="MD" & data$ruleYear==2026
  ...
  # ee)
  if(2026 %in% unique(data$ruleYear) & "OH" %in% unique(data$stateAbbrev)){
    
    # Adjust Personal & Dependent Exemptions depending on income thresholds
    data$PersonalExemption <- case_when(data$income.base > data$OH_Threshold2 & data$stateAbbrev=="OH" & data$ruleYear==2026 ~ data$OH_Exemption2
                                        ,data$income.base > data$OH_Threshold1 & data$income.base <= data$OH_Threshold2 & data$stateAbbrev=="OH" & data$ruleYear==2026  ~ data$OH_Exemption1
                                        ,data$income.base <= data$OH_Threshold1 & data$stateAbbrev=="OH" & data$ruleYear==2026 ~ data$PersonalExemption
                                        ,TRUE ~ data$PersonalExemption)
    
    data$DependentExemption <- case_when(data$income.base > data$OH_Threshold2 & data$stateAbbrev=="OH" & data$ruleYear==2026 ~ data$OH_Exemption2
                                         ,data$income.base > data$OH_Threshold1 & data$income.base <= data$OH_Threshold2 & data$stateAbbrev=="OH" & data$ruleYear==2026~ data$OH_Exemption1
                                         ,data$income.base <= data$OH_Threshold1 & data$stateAbbrev=="OH" & data$ruleYear==2026~ data$DependentExemption
                                         , TRUE ~ data$DependentExemption)
  }
  
  # ff)
  if(2026 %in% unique(data$ruleYear) & "OR" %in% unique(data$stateAbbrev)){
  ...
  # gg)
  if(2026 %in% unique(data$ruleYear) & "RI" %in% unique(data$stateAbbrev)){
    
    # Condition for standard deduction phase out
    subset0 <- data$income.base > data$RI_Threshold1 & data$stateAbbrev=="RI" & data$ruleYear==2026
  ...
  # hh)
  if(2026 %in% unique(data$ruleYear) & "UT" %in% unique(data$stateAbbrev)){
    
    # Condition for standard deduction phase out
    subset0 <- data$income.base > data$UT_Threshold1 & data$stateAbbrev=="UT" & data$ruleYear==2026
  ...
  # jj)
  if(2026 %in% unique(data$ruleYear) & "WI" %in% unique(data$stateAbbrev)){
    
    # Condition for standard deduction phase out
    subset0 <- data$income.base > data$WI_Threshold1 & data$stateAbbrev=="WI" & data$ruleYear==2026
  ...
  # initalize
  data$AdditionalTaxTotal <- 0
  if("CT" %in% unique(data$stateAbbrev) & 2025 %in% unique(data$ruleYear)){
  ...
  if("CT" %in% unique(data$stateAbbrev) & 2024 %in% unique(data$ruleYear)){
...
function.fedctc<-function(data
  ...
  if(2026 %in% unique(data$ruleYear)){ # make sure that year is in the list
    
    temp<-data[data$ruleYear==2026,]
...
function.statectc <- function(data,
  ...
  if (2025 %in% unique(full_data$ruleYear)) {
    ...
    rules <- statectcData %>% dplyr::filter(ruleYear == 2025)
    ...
    data_nonmn <- left_join(data_nonmn, rules %>%
                              filter(is.na(MNEligibleKids)),
                            by = c("stateFIPS", "FilingStatus"))
    
    data_mn <- left_join(data_mn, rules %>%
                           filter(stateFIPS == 27),
                         by = c("stateFIPS", "FilingStatus",
    ...
    # ===============================
    # Percent of Federal CTC States: OK
    # ===============================
    ...
    non_refundable_2 <- data$Refundable == "No" & !is.na(data$Refundable) & (ok_mask)
    ...
    data_colorado <- data %>%
      filter(stateFIPS == 8) %>%
      left_join(rules %>% filter(stateFIPS == 8), by = c("stateFIPS", "FilingStatus"))
    ...
    # Rules for Maine and Colorado special where families with kids above 
    # Age limit get a larger credit 
    
    # =============================
    # Maine special rules
    # ============================
    me_mask <- data$stateFIPS == 23
    
    if (any(me_mask, na.rm = TRUE)) {
      
      me_kids_0_5  <- rowSums(age_matrix[me_mask, , drop = FALSE] >= 0 & age_matrix[me_mask, , drop = FALSE] <= 5,  na.rm = TRUE)
      me_kids_6_16 <- rowSums(age_matrix[me_mask, , drop = FALSE] >= 6 & age_matrix[me_mask, , drop = FALSE] <= 16, na.rm = TRUE)
      
      me_base_credit <- me_kids_0_5  * data$ValueBin1_0_5[me_mask] +
        me_kids_6_16 * data$ValueBin1_6_16[me_mask]
      
      me_value <- me_base_credit
      
      me_phaseout_mask <- !is.na(data$PhaseoutRatePer1000[me_mask]) &
        (data$income.base[me_mask] > data$IncomeBin1Max[me_mask])
      
      if (any(me_phaseout_mask, na.rm = TRUE)) {
        excess_income <- data$income.base[me_mask][me_phaseout_mask] - data$IncomeBin1Max[me_mask][me_phaseout_mask]
        phaseout_amt  <- (excess_income / 1000) * data$PhaseoutRatePer1000[me_mask][me_phaseout_mask] * me_base_credit[me_phaseout_mask]
        me_value[me_phaseout_mask] <- pmax(me_base_credit[me_phaseout_mask] - phaseout_amt, 0)
      }
      
      data$value.statectc[me_mask] <- me_value
    }
    
    # ===============================
    #  New York Special Rules 
    # =======================================
    ny_mask_all <- data$stateFIPS == 36
    
    if (any(ny_mask_all, na.rm = TRUE)) {
      
      ny_kids_0_3  <- rowSums(age_matrix[ny_mask_all, , drop = FALSE] >= 0 & age_matrix[ny_mask_all, , drop = FALSE] <= 3,  na.rm = TRUE)
      ny_kids_4_16 <- rowSums(age_matrix[ny_mask_all, , drop = FALSE] >= 4 & age_matrix[ny_mask_all, , drop = FALSE] <= 16, na.rm = TRUE)
      
      ny_base_credit <- ny_kids_0_3  * data$ValueBin1_0_3[ny_mask_all] +
        ny_kids_4_16 * data$ValueBin1_4_16[ny_mask_all]
      
      ny_value <- ny_base_credit
      
      ny_phaseout_mask <- !is.na(data$PhaseoutRatePer1000[ny_mask_all]) &
        (data$income.base[ny_mask_all] > data$IncomeBin1Max[ny_mask_all])
      
      if (any(ny_phaseout_mask, na.rm = TRUE)) {
        excess_income <- data$income.base[ny_mask_all][ny_phaseout_mask] - data$IncomeBin1Max[ny_mask_all][ny_phaseout_mask]
        phaseout_amt  <- (excess_income / 1000) * data$PhaseoutRatePer1000[ny_mask_all][ny_phaseout_mask] * ny_base_credit[ny_phaseout_mask]
        ny_value[ny_phaseout_mask] <- pmax(ny_base_credit[ny_phaseout_mask] - phaseout_amt, 0)
      }
      
      data$value.statectc[ny_mask_all] <- ny_value
    }
  ...
  if (2024 %in% unique(full_data$ruleYear)) {
    ...
    rules <- statectcData %>% dplyr::filter(ruleYear == 2024)
...
function.statecdctc<-function(data
  ...
  chosen_bin <- max.col(in_bin_matrix, ties.method = "first")
  standard_valid_bin <- rowSums(in_bin_matrix, na.rm = TRUE) > 0
  standard_chosen_bin <- chosen_bin
  standard_chosen_bin[!standard_valid_bin] <- NA_integer_
  standard_pfed_mat <- as.matrix(data_main[, percent_fed_cols])
  standard_pfed_mat[is.na(standard_pfed_mat)] <- 0
  ...
  if (length(ma_rows) > 0) {
    ...
    data_main$value.statecdctc[ma_rows[nonrefundable]] <- pmin(
      ...
      ma_data$stateincometax[nonrefundable],
      na.rm = TRUE
    )
  }
